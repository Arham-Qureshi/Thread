...
struct Node    { std::string id, type, subtype, role, language, content; };
struct Edge    { std::string source, target, relation; };

static std::vector<Message> parseMessages(const std::string& json) {
    std::vector<Message> msgs;
    size_t pos = 0;
    while (pos < json.size() && json[pos] != '[') pos++;
    if (pos >= json.size()) return msgs;
    pos++;
    ...
    while (pos < json.size()) {
        ...
        while (pos < json.size() && json[pos] != '}') {
            while (pos < json.size() && json[pos] != '"') { if (json[pos] == '}') goto done; pos++; }
            pos++;
            ...
            if (key == "role") msg.role = val;
            else if (key == "content") msg.content = val;
            while (pos < json.size() && json[pos] != ',' && json[pos] != '}') pos++;
            if (json[pos] == ',') pos++;
        }
        done:
...
struct CodeBlock { std::string language, code; };
...
static std::vector<std::string> extractLinks(const std::string& content) {
    ...
    while (pos < content.size()) {
        ...
        if (url.size() > 4) links.push_back(url);
        pos = urlEnd + 1;
    }
    pos = 0;
    while (pos < content.size()) {
        ...
        if (content.substr(h, 8) != "https://" && content.substr(h, 7) != "http://") { pos = h + 4; continue; }
        size_t end = h;
        while (end < content.size() && content[end] != ' ' && content[end] != '\n' && content[end] != ')' && content[end] != '"') end++;
        std::string url = content.substr(h, end - h);
        bool dup = false;
        for (auto& l : links) if (l == url) { dup = true; break; }
        if (!dup) links.push_back(url);
        pos = end;
    }
    return links;
}
...
static std::vector<std::string> extractBoldVars(const std::string& content) {
    ...
    while (pos < content.size()) {
        ...
        if (!text.empty() && text.size() < 80) vars.push_back(text);
        pos = e + 2;
    }
    return vars;
}

static std::string serializeGraph(const std::vector<Node>& nodes, const std::vector<Edge>& edges) {
    std::string j = "{\"nodes\":[";
    for (size_t i = 0; i < nodes.size(); i++) {
        if (i) j += ",";
        j += "{\"id\":\"" + jsonEscape(nodes[i].id) + "\"";
        j += ",\"type\":\"" + jsonEscape(nodes[i].type) + "\"";
        if (!nodes[i].subtype.empty())  j += ",\"subtype\":\"" + jsonEscape(nodes[i].subtype) + "\"";
        if (!nodes[i].role.empty())     j += ",\"role\":\"" + jsonEscape(nodes[i].role) + "\"";
        if (!nodes[i].language.empty()) j += ",\"language\":\"" + jsonEscape(nodes[i].language) + "\"";
...
const char* buildGraph(const char* jsonInput) {
    ...
    for (auto& msg : messages) {
        ...
        for (auto& bv : extractBoldVars(msg.content)) {
            std::string aid = "a" + std::to_string(aIdx++);
            nodes.push_back({aid, "artifact", "variable", "", "", bv});
            edges.push_back({tid, aid, "mentions"});
        }
    }

    return returnString(serializeGraph(nodes, edges));
}
